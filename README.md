...
The primary API for fetching market data and updating market positions is provided via [alpaca-py](https://alpaca.markets). Statistical modeling (e.g., cointegration tests) is seamlessly integrated with [statsmodels](https://www.statsmodels.org/stable/index.html). Otherwise, a quick install looks something like:

```bash
pip install alpaca-py matplotlib numpy pandas python-dotenv statsmodels 
```

### 1. Setup Environment

Create (or modify the existing) `.env` file and set these environment variables yourself:
...
The file `.env_example` can be used as a template for creating the required `.env` file.
...
### Trading Parameters
The trading logic can be customized by modifying the global constants in `trading_bot.py`:

```python
# Current thresholds (can be adjusted)
MOMENTUM_BUY_THRESHOLD = 0.02    # 2% price increase
MOMENTUM_SELL_THRESHOLD = -0.02  # 2% price decrease
MOMENTUM_MIN_DATA_POINTS = 10    # Minimum data for signals
```

## 📝 Logging

Real-time status, signals, and trades are printed to the console. **All** activities are logged to `live_trading.log` with timestamps: (i) Trading decisions, (ii) market data updates, (iii) error messages, and (iv) performance metrics.
